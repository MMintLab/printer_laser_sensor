...
def load_images(input_filenames, output_filenames, dataset_folder):
    ...
    for i in range(len(input_filenames)):
        ...
        input_image = input_image[0:input_image.shape[0]-0, :, :]
        output_image = cv2.imread(dataset_folder+output_filenames[i])
        #crop bottom of image by 10 pixels
        output_image = output_image[0:output_image.shape[0]-0, :, :]
...
def model_output(input_image, weights):
    ...
    weighted_average = get_weighted_average(weights, red_channel, green_channel, blue_channel, sat_channel, lum_channel, value_channel)
