...
def get_weighted_average(weights, array1, array2, array3):
    weighted_average = (weights[0,0]*array1 + weights[1,0]*array2 + weights[2,0]*array3)/np.sum(weights)
...
def gradient_descent(weights, red_channel, green_channel, blue_channel, incr, learning_rate, output_image):
    weighted_average = get_weighted_average(weights, red_channel, green_channel, blue_channel)
    ...
    weights1 = weights+np.array([incr,0,0]).reshape(3,1)
    #print(weights1)
    #input("Press Enter to continue...")
    weights2 = weights+np.array([0,incr,0]).reshape(3,1)
    #print(weights2)
    #input("Press Enter to continue...")
    weights3 = weights+np.array([0,0,incr]).reshape(3,1)
    #print(weights3)
    #input("Press Enter to continue...")
    red_cost = compute_cost(get_highest_intensity_pixel_indices(get_weighted_average(weights1, red_channel, green_channel, blue_channel)), output_indices)
    green_cost = compute_cost(get_highest_intensity_pixel_indices(get_weighted_average(weights2, red_channel, green_channel, blue_channel)), output_indices)
    blue_cost = compute_cost(get_highest_intensity_pixel_indices(get_weighted_average(weights3, red_channel, green_channel, blue_channel)), output_indices)
    cost_array = np.array([red_cost, green_cost, blue_cost])
    gradient_array = (cost_array-orig_cost)/incr
    #print(cost_array)
    #input("Press Enter to continue...")
    #print(gradient_array)
    #input("Press Enter to continue...")
    new_weights = weights - (learning_rate * incr * gradient_array.reshape(3, 1))
    #print(new_weights)
    #input("Press Enter to continue...")
    return new_weights
...
weighted_average1 = get_weighted_average(weights, red_image, green_image, blue_image)
...
try:
    while abs(cost-prevcost) > 0.0001*prevcost:
        print(cost)
        prevcost = cost
        weights = gradient_descent(weights, red_image, green_image, blue_image, incr, learning_rate, output_image)
        weighted_average1 = get_weighted_average(weights, red_image, green_image, blue_image)
        output_indices1 = get_highest_intensity_pixel_indices(output_image)
        input_indices1 = get_highest_intensity_pixel_indices(weighted_average1)
        cost = compute_cost(input_indices1, output_indices1)
...
for i in range(len(input_filenames)):
    ...
    #save image
    cv2.imwrite('trained'+output_filenames[i], weighted_average)
    #print(i)